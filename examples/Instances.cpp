...
{
    VkayContextInit("Boids", 1280, 720);
    ...
    for (size_t i = 0, j = 0; i < SPRITE_COUNT; i++) {
        ...
        sprite.transform.position = { _x - 10000.f, _y-2000, .0f };
    ...
    // quad.mesh.vertices[0].color = {255,255,255, 6};
    // quad.mesh.vertices[1].color = {255,255,255, 6};
    // quad.mesh.vertices[2].color = {255,255,255, 6};
    // quad.mesh.vertices[3].color = {255,255,255, 6};
...
void UpdateAndRender()
{
    ...
    while (!bQuit) {
        ...
        vkay::InstancesBucketDraw(vkr.frames[vkr.frame_idx_inflight].cmd_buffer_gfx, &vkr, &instances, quad.mesh);
