...
int main(int argc, char* argv[])
{
	...
	VK_CHECK(vkCreatePipelineLayout(vkr.device, &pipeline_layout_info, NULL, &pipeline_layout));
	...
	VkayCameraCreate(&vkr, pipeline_layout, &camera);
	...
	triangle_transform.position = { 0,0,1 };
...
void UpdateAndRender()
{
	...
	while (!bQuit) {
		...
		camera.m_position = { camera_x, camera_y - 0.f, camera_z - 320.f };
		///////////////////////////////
		// Compute dispatch
		...
		vkCmdBindDescriptorSets(cmd_buffer_gfx, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline_layout, 0, 1, &camera.m_set_global[vkr.frame_idx_inflight], 0, NULL);
		VkayCameraUpdate(&vkr, &camera);
		vkCmdPushConstants(cmd_buffer_gfx, pipeline_layout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(Transform), &triangle_transform);
