...
//         // instanced rendering. We have to be able to use texture without the need for *vkr
//         void Create(const char *filepath, VkayRenderer *vkr, VkayContext vkc)
//         {
//             id = vkr->texture_array_count;
...
//             VKCHECK(vmaCreateImage(vkr->allocator, &ci_image, &ci_allocation, &image, &allocation, NULL));
...
//             VKCHECK(vkCreateImageView(vkr->device, &ci_image_view, NULL, &view));
...
//             // VKCHECK(vmaCreateBuffer(vkr->allocator, &bufferInfo, &vmaallocInfo, &staging_buffer.buffer, &staging_buffer.allocation, NULL));
...
//             vkCmdCopyBufferToImage(vkr->frames[0].cmd_buffer_gfx, staging_buffer.buffer, image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &buffer_image_copy);
...
//         void Destroy(VkayRenderer *vkr, VkayContext vkc)
//         {
//             vkDestroyImageView(vkr->device, view, NULL);
//             vmaDestroyImage(vkr->allocator, image, allocation);
//         }
//     };
// }
